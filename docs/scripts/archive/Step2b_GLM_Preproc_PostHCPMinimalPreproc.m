function [output] = Step2b_GLM_Preproc_PostHCPMinimalPreproc(SUBJECTLIST)
...
%2. GLMs to RUN: 3 Trial Epoch variations (varblock, varepochlength, varepochRT) x 2
%Task regressor variations (1Taskreg , 64Taskregs) x 2 Sessions (Prac,
%Test; 6 stimfile.mat files x 2 prac/test sessions in each)
%x 2 Preproc types (MSMsulc, MSMsulc+MSMall)  = 24 total
%i.e. Model(1a-c,2a-c = 6) x MSM(sulc,sulc+all = 2) x Session(prac,test=2)= 24

%3. **Accommodate missing/aborted scans!

%4. **Separate GLMs for Practice and Test session - makes sense, as any
%commonalities between Prac/Test of interest (i.e. learning,
%representation) will then be suppressed in a common GLM
%*Might have to loop GLM script below for j=1:num_sessions...


%OVERALL PLAN (*Focus on Model1a-c comparisons primarily for now; run
%Model2 if there's time):
...
%addpath('/projects/IndivRITL/docs/scripts/')
...
%Basic data parameters - **separate for Prac and Test session; affected by
%aborted runs for each subject
%RUNNAMES is determined by names given to func runs by HCP preproc...
%Practice
pracinfo=struct;
pracinfo.RUNNAMES = {'Rest_Practice','Task_Practice1','Task_Practice2','Task_Practice3','Task_Practice4','Task_Practice5','Task_Practice6','Task_Practice7','Task_Practice8','Task_Practice9','Task_Practice10','Task_Practice11','Task_Practice12'};
pracinfo.numRuns=length(pracinfo.RUNNAMES);
pracinfo.RESTRUNS=1;
pracinfo.TASKRUNS=2:13;
pracinfo.RUNLENGTHS = [1147, 521, 521, 521, 521, 521, 521, 521, 521, 521, 521, 521, 521];
%Test
testinfo=struct;
testinfo.RUNNAMES = {'Rest_Test','Task_Test1','Task_Test2','Task_Test3','Task_Test4','Task_Test5','Task_Test6','Task_Test7','Task_Test8','Task_Test9','Task_Test10'};
testinfo.numRuns=length(testinfo.RUNNAMES);
testinfo.RESTRUNS=1;
testinfo.TASKRUNS=2:11;
testinfo.RUNLENGTHS = [1147, 521, 521, 521, 521, 521, 521, 521, 521, 521, 521];
...
%Start loops: model->MSM version->session
GLMOutput=struct;
execute=1;%set to 0 if GLM has already been run

if execute==1
    for model=4:6 %length(stim_model_input)
        for MSM_version=1:length(MSM_input)
            for session=1:length(session_input) %prac, test
                %set names (eg for output name)
                model_name=stim_model_input{model};
                model_mat=stim_model_mat{model};
                MSM_name=MSM_struct_name{MSM_version};
                session_name=session_input{session};
                
                %set
                if session==1 %Prac
                    func_info=pracinfo;
                elseif session==2 %Test
                    func_info=testinfo;
                end
                ...
                for subjIndex=1:numSubjs
                    ...
                    %**modify parms for subjects with aborted runs
                    %Subject 6 Test: use scans 1 to 8 (totalRuns=8)
                    %Subject 8 Test: use scans 1 to 5
                    %Subject 9 Prac: use scans 1 to 8
                    if session==1
                        if strcmp(subjNum,'sub-9')
                            func_info.RUNNAMES = {'Rest_Practice','Task_Practice1','Task_Practice2','Task_Practice3','Task_Practice4','Task_Practice5','Task_Practice6','Task_Practice7','Task_Practice8'};
                            ...
                            func_info.RUNNAMES = {'Rest_Practice','Task_Practice1','Task_Practice2','Task_Practice3','Task_Practice4','Task_Practice5','Task_Practice6','Task_Practice7','Task_Practice8','Task_Practice9','Task_Practice10','Task_Practice11','Task_Practice12'};
                            func_info.numRuns=length(func_info.RUNNAMES);
                            func_info.RESTRUNS=1;
                            func_info.TASKRUNS=2:13;
                            func_info.RUNLENGTHS = [1147, 521, 521, 521, 521, 521, 521, 521, 521, 521, 521, 521, 521];
                            
                        end
                        
                    elseif session==2
                        if strcmp(subjNum,'sub-1')
                            func_info.RUNNAMES = {'Rest_Test','Task_Test2','Task_Test3','Task_Test4','Task_Test5','Task_Test6','Task_Test7','Task_Test8','Task_Test9','Task_Test10','Task_Test11'};
                            func_info.numRuns=length(func_info.RUNNAMES);
                            func_info.RESTRUNS=1;
                            func_info.TASKRUNS=2:11;
                            func_info.RUNLENGTHS = [1147, 521, 521, 521, 521, 521, 521, 521, 521, 521, 521];
                        elseif strcmp(subjNum,'sub-6')
                        ...
                        elseif strcmp(subjNum,'sub-8')
                            func_info.RUNNAMES = {'Rest_Test','Task_Test1','Task_Test2','Task_Test3','Task_Test4','Task_Test5'};
                            func_info.numRuns=length(func_info.RUNNAMES);
                            func_info.RESTRUNS=1;
                            func_info.TASKRUNS=2:6;
                            func_info.RUNLENGTHS = [1147, 521, 521, 521, 521, 521];
                            
                        else
                            func_info.RUNNAMES = {'Rest_Test','Task_Test1','Task_Test2','Task_Test3','Task_Test4','Task_Test5','Task_Test6','Task_Test7','Task_Test8','Task_Test9','Task_Test10'};
                            func_info.numRuns=length(func_info.RUNNAMES);
                            func_info.RESTRUNS=1;
                            func_info.TASKRUNS=2:11;
                            func_info.RUNLENGTHS = [1147, 521, 521, 521, 521, 521, 521, 521, 521, 521, 521];
                    ...
                    subjTemporaryAnalysisDir=[SUBJDIROUTPUT '/analysis_' ANALYSISNAME '/' model_name '_' MSM_name '_' session_name '/'];
                    ...
                    for runName=func_info.RUNNAMES
                        ...
                        tseriesMatSubj(1:180,1:func_info.RUNLENGTHS(runCount),runCount)=L_dat.cdata(:,1:func_info.RUNLENGTHS(runCount));
                        tseriesMatSubj(181:end,1:func_info.RUNLENGTHS(runCount),runCount)=R_dat.cdata(:,1:func_info.RUNLENGTHS(runCount));
                    ...
                    savedfile=[subjTemporaryAnalysisDir mfilename '_' ANALYSISNAME '_nuisanceTSVars.mat'];
                    ...
                    savedfile=[subjTemporaryAnalysisDir mfilename '_' ANALYSISNAME '_TaskRegressorVars.mat'];
                    ...
                    if and(~isempty(func_info.TASKRUNS), or(RERUN_TASKREGRESSORPREP == 1, ~exist(savedfile, 'file')))
                        ...
                        %init vars
                        totalTRs=sum(func_info.RUNLENGTHS(2:end)); %only applies to task runs, hence exclude run1
                        taskdesignmat=[];
                        
                        %set input_timingfile based on model
                        %event timings are stored in a .mat file,
                        %containing a struct
                        input_timingfile=[timingfileDir,'/',model_name,'_',subjNum,'.mat'];
                        load(input_timingfile);
                        %rename
                        eval(['stim_timing=',model_mat,';']);
                        eval(['clear ',model_mat]);
                        
                        %pull out prac or test based on session
                        reg_info=stim_timing.(session_name).Regs;
                        
                        %find number of task regs (find number of struct fields)
                        %2 for model1 (1 Trial + 1 Enc); 8 for Model2 Prac
                        %(4 tasks x trial/enc), 128 for Model2 Test (64
                        %tasks x trial/enc)
                        reg_names=fieldnames(reg_info);
                        num_regs=length(reg_names);
                        
                        %extract timing info for each run for each reg
                        %should end up with one binary col for each task reg (with
                        %rows = concatenated TRs)
                        for regs=1:num_regs
                            reg_loop=reg_info.(reg_names{regs}).reg;%pick out regressors: rows=runs, cols=blocks
                            
                            %run_regressor is the column coding for each
                            %reg; 1s are added for each Run
                            run_regressor=zeros(totalTRs,1); %initialize for each run
                            %set start_TR based on runNum (given that regressors
                            %have to be written for concatenated TRs)
                            start_TR=0;
                            
                            for runNum=1:length(func_info.TASKRUNS)
                                %pick out run
                                reg_run=reg_loop(runNum,:);
                                %remove empty cells
                                reg_run=reg_run(~cellfun('isempty',reg_run));
                                
                                %Split strings into onset,duration cols
                                timing_run=[];
                                for event=1:length(reg_run)
                                    event_loop=reg_run(event);
                                    event_loop=strsplit(event_loop{1},':');
                                    event_num=[];
                                    event_num=[event_num,str2num(event_loop{1,1})];
                                    event_num=[event_num,str2num(event_loop{1,2})];
                                    timing_run=[timing_run;event_num];
                                end
                                
                                
                                if ~isempty(timing_run) %sum events do not appear during a run e.g. Model2
                                    block_secs=[timing_run(:,1),timing_run(:,1)+timing_run(:,2)];%onset and offset
                                    block_TRs=[floor(block_secs(:,1)/TR_INSECONDS),ceil(block_secs(:,2)/TR_INSECONDS)]; %round DOWN for onset, round UP for offset
                                    %convert onsets and offsets to TR indices (onset:offset)
                                    block_TRs_stim=[];
                                    for xx=1:size(block_TRs,1)
                                        xx_indx=[block_TRs(xx,1):block_TRs(xx,2)];
                                        block_TRs_stim=[block_TRs_stim,xx_indx];
                                    end
                                    %adjust block_TRs_stim according to start_TR
                                    block_TRs_stim=block_TRs_stim+start_TR;
                                    run_regressor(block_TRs_stim)=1;%add to run_regressor
                                end
                                start_TR=start_TR+func_info.RUNLENGTHS(runNum+1); %adjust TR count
                                
                            end
                            %add run_regressor to rundesignmat (array
                            %coding for each task regressor in design
                            %matrix
                            if ~isempty(find(run_regressor~=0)) %*checks if the entire regressor is 0 e.g. for aborted subjects
                                taskdesignmat=[taskdesignmat,run_regressor];
                            end
                    ...
                    savedfile=[subjTemporaryAnalysisDir mfilename '_' ANALYSISNAME '_RestNuisanceGLMVars.mat'];
                    ...
                    if and(~isempty(func_info.RESTRUNS), or(RERUN_RESTGLM == 1, ~exist(savedfile, 'file')))
                    ...
                    %Only execute this if RERUN_TASKGLM==1 or the savedfile doesn't exist for this subject, but skip if no TASKRUNS (i.e., only rest data are included)
                    if and(~isempty(func_info.TASKRUNS), or(RERUN_TASKGLM == 1, ~exist(savedfile, 'file')))
                    ...
                    %% Apply temporal filtering (and motion scrubbing)

                    %Set to 1 if you want to run this procedure again for subjects that already had it run before (otherwise it will load from previously saved files)
                    RERUN_TEMPORALFILTER=1;

                    savedfile=[subjTemporaryAnalysisDir mfilename '_' ANALYSISNAME '_TemporalFilter.mat'];
                        ...
                        if or(RERUN_TEMPORALFILTER == 1, ~exist(savedfile, 'file'))
                            ...
                            if ~isempty(func_info.RESTRUNS)
                                ...
                                if TEMPORALFILTER==1
                                    ...
                                    disp(['Saving results to: ' savedfile])
                                    save(savedfile, 'scrubbedDataOutput')
                                end
...
%                         end
            ...
            GLMOutput.(model_name).(MSM_name).(session_name)=output;
...
%% *then work on analysis steps after GLM is run!
