...
close all
filename=load('/projects3/NeuralMech/data/results/GLM/ActivationStats.mat');
struct=sig_regions(1);
...
glasser_stats = zeros(nVertices,num_statVars); % We're going to map your 'significant statistic' back onto this array. Note that you can have more than
... one column if you wish (e.g., for t-statistic, p-values, etc.)
...
outputcifti = glasser; % Create a new cifti data type (using the original glasser atlas variable)
...
outputfilename = 'output.dscalar' % you can change this name as you wish
